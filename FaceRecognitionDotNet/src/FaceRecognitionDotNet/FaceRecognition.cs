...
namespace FaceRecognitionDotNet
{
    ...
    public sealed class FaceRecognition : IDisposable
    {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="knownFaceEncodings"/> or <paramref name="faceEncodingToCheck"/> is null.</exception>
        /// <exception cref="ObjectDisposedException"><paramref name="faceEncodingToCheck"/> is disposed. Or <paramref name="knownFaceEncodings"/> contains disposed object.</exception>
        public static IEnumerable<double> CompareFaces(IEnumerable<FaceEncoding> knownFaceEncodings, FaceEncoding faceEncodingToCheck)
        {
            if (knownFaceEncodings == null)
                throw new ArgumentNullException(nameof(knownFaceEncodings));
            if (faceEncodingToCheck == null)
            ...
            foreach (var faceEncoding in array)
                yield return FaceDistance(faceEncoding, faceEncodingToCheck);
        }
        ...
        /// <exception cref="ArgumentNullException"><paramref name="knownFaceEncodings"/> or <paramref name="faceEncodingToCheck"/> is null.</exception>
        /// <exception cref="ObjectDisposedException"><paramref name="faceEncodingToCheck"/> is disposed. Or <paramref name="knownFaceEncodings"/> contains disposed object.</exception>
        public static IEnumerable<FaceEncodingData> CompareFaces(IEnumerable<FaceEncoding> knownFaceEncodings, FaceEncoding faceEncodingToCheck, double tolerance = 0.6d)
        {
            if (knownFaceEncodings == null)
                throw new ArgumentNullException(nameof(knownFaceEncodings));
            if (faceEncodingToCheck == null)
            ...
            foreach (var faceEncoding in array) {
                double distance = FaceDistance(faceEncoding, faceEncodingToCheck);
                if (distance <= tolerance) {
                    yield return  new FaceEncodingData(faceEncoding, distance);
                }
            }
        }
        ...
        /// <returns></returns>
        public FaceEncoding FaceEncodingTop1(Image image, IEnumerable<Location> knownFaceLocation = null, int numJitters = 1) {
            if (image == null)
                throw new ArgumentNullException(nameof(image));
            if (image.IsDisposed)
                throw new ObjectDisposedException(nameof(image));
        ...
        public static Image LoadImage(Matrix<RgbPixel> matrix)
        {
            if (matrix == null)
                throw new ArgumentNullException(nameof(matrix));

            return new Image(matrix, Mode.Rgb);
        }
        /// <summary>
